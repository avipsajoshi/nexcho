...
function HomeComponent() {
			...
			const response = await fetch(
				`${server_url}/api/v1/users/create_meeting`,
				{
					...
					body: JSON.stringify({
						...
						meetingCode: code,
						meetingTitle: oneTapMeetingTitle.trim() || "One-Tap Meeting",
					}),
				}
			);
			if (response.ok) {
				...
				setMessage({
					...
					text:
						errorData.message ||
						`Failed to create meeting: Status ${response.status}`,
				});
			}
		} catch (error) {
	...
	const handleJoinCreatedMeeting = async () => {
		...
		try {
			...
			const response = await fetch(
				`${server_url}/api/v1/users/join_created_meeting`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
			...
			if (response.ok) {
				...
				setMessage({
					...
					text:
						errorData.message ||
						`Failed to join created meeting: Status ${response.status}`,
				});
			}
		} catch (error) {
			console.error("Fetch error:", error);
	...
	const handleScheduleMeeting = async () => {
		...
		const meetingCode = Math.random()
			...
			.toUpperCase();
		try {
			const res = await fetch(`${server_url}/api/v1/users/create_meeting`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					token: localStorage.getItem("token"),
						...
						<section className="bg-white rounded-2xl shadow-lg p-6">
							...
							<div className="mb-4">
								<TextField
									...
									variant="outlined"
									fullWidth
									value={oneTapMeetingTitle}
									onChange={(e) => {
										setOneTapMeetingTitle(e.target.value);
									}}
								/>
							</div>
									...
									.map((m) => {
										...
										return (
											...
											>
												...
												<div className="flex items-center gap-2">
													<Tooltip title="Open analytics in new tab">
														<IconButton
															size="small"
															onClick={() =>
