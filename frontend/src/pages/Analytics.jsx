...
export default function MeetingDetails() {
  ...
  const videoSrc =
    (meeting.recording && `${server}/uploads/recordings/${meeting.recording}`) ||
    meeting.recording ||
    "";
  ...
  const attendance = Array.isArray(meeting.attendance) && meeting.attendance.length
    ? meeting.attendance
    : [
        { name: "Unit 1", percent: 80 },
        { name: "Unit 2", percent: 50 },
      ];
  ...
  const downloadSummary = () => {
    ...
    a.href = url;
    a.download = `${(meeting.title || "meeting").replace(/\s+/g, "_")}_summary.txt`;
    a.click();
      ...
      <div className="col-span-1 flex flex-col gap-4">
        <Card className="shadow-xl rounded-2xl">
          <CardContent className="p-4">
            ...
            <p className="text-sm"><strong>Title:</strong> {meeting.title || "One Time Meeting"}</p>
            <p className="text-sm"><strong>Date:</strong> {meeting.date || "—"}</p>
            <p className="text-sm"><strong>Duration:</strong> {meeting.duration || "00:07:03"}</p>
            <p className="text-sm"><strong>Host:</strong> {meeting.host || "Unknown"}</p>
            <p className="text-sm"><strong>Participants:</strong> {meeting.participants?.length ?? "—"}</p>
          </CardContent>
        </Card>
            ...
            <div className="relative bg-black rounded-t-2xl overflow-hidden">
              ...
              <div className="absolute left-4 right-4 bottom-4">
                <div className="flex items-center justify-between text-xs text-white/90 mb-1">
                  ...
                  <span>{fmt(currentTime)} / {fmt(duration || meeting.duration || 0)}</span>
                </div>
            ...
            <div className="p-4">
              <div className="flex justify-between items-start gap-4">
                <div className="text-sm space-y-1">
                  <p><strong>Title:</strong> {meeting.title || "One Time Meeting"}</p>
                  <p><strong>Host:</strong> {meeting.host || "Unknown"}</p>
                </div>
                <div className="text-sm text-neutral-500">
                  <p><strong>Date:</strong> {meeting.date || "—"}</p>
                  <p><strong>Duration:</strong> {meeting.duration || fmt(duration || 0)}</p>
                </div>
              </div>
            </div>
          </CardContent>
              ...
              {attendance.map((u, i) => (
                <div key={i}>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-xs text-neutral-500">{u.percent}%</span>
                  </div>
