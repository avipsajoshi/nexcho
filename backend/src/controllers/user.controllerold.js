...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const meetingId = req.body.meetingId  || "unknownmeetings";
    const ext = path.extname(file.originalname) || ".webm";

    const filePath = path.join(recordingsDir, `${meetingId}${ext}`);
...
const joinMeeting = async (req, res) => {
  try {
    ...
    // Condition: Host starts meeting
    if (!meeting.joinedAt && meeting.user_id.equals(user._id)) {
      meeting.joinedAt = new Date();
      await meeting.save();
      return res.status(200).json({ message: "Host has started the meeting" });
    }

    // Condition: Meeting not started, and user is not host
    if (!meeting.joinedAt && !meeting.user_id.equals(user._id)) {
      return res.status(400).json({ message: "Meeting not started. Please wait for host." });
    }

    return res.status(400).json({ message: "Unexpected condition occurred" });
  } 
  catch (error) {
    console.error("Error joining meeting:", error);
